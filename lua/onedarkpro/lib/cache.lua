...
local function set_highlights_block(block, sort)
    ...
    logger.debug("CACHE: Generating editor highlight groups")
    highlight_block = highlights.create(groups.editor, highlight_block)
    logger.debug("CACHE: Generating syntax highlight groups")
    highlight_block = highlights.create(groups.syntax, highlight_block)
    logger.debug("CACHE: Generating filetype highlight groups")
    highlight_block = highlights.create(groups.filetypes, highlight_block)
    logger.debug("CACHE: Generating plugin highlight groups")
    highlight_block = highlights.create(groups.plugins, highlight_block)
    logger.debug("CACHE: Generating custom highlight groups")
...
local function write_cache(contents)
    ...
    logger.debug("CACHE: Opening cache file for writing")
    local file = io.open(cache_file(), "w")
    if file then
        logger.debug("CACHE: Writing to cache file")
        file:write(table.concat(contents, "\n"))
        logger.debug("CACHE: Closing cache file")
        file:close()
    end
end
...
---@return nil
function M.clean()
    logger.debug("CACHE: Preparing to clean")
    os.remove(cache_file())
    logger.debug("CACHE: Completed clean")
end
