...
T["Using the theme"]["it creates the autocmds for inactive windows"] = function()
    ...
    local has_winenter =
        child.lua_get([[string.find(vim.api.nvim_exec("autocmd Onedarkpro", true), "WinEnter") ~= nil]])
    local has_winleave =
        child.lua_get([[string.find(vim.api.nvim_exec("autocmd Onedarkpro", true), "WinLeave") ~= nil]])
